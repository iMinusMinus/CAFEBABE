...
final class ChecksumHashFunction extends AbstractHashFunction implements Serializable {
    private final /* Immutable */ Supplier<? extends Checksum> checksumSupplier;
    private final int bits;
    private final String toString;

    ChecksumHashFunction(
            /* Immutable */ Supplier<? extends Checksum> checksumSupplier, int bits, String toString) {
        this.checksumSupplier = Objects.requireNonNull(checksumSupplier); // checkNotNull(checksumSupplier);
//        checkArgument(bits == 32 || bits == 64, "bits (%s) must be either 32 or 64", bits);
        if (bits != 32 && bits != 64) {
            throw new IllegalArgumentException(String.format("bits (%s) must be either 32 or 64", bits));
        }
        this.bits = bits;
        this.toString = Objects.requireNonNull(toString); // checkNotNull(toString);
    }
    ...
    private final class ChecksumHasher extends AbstractByteHasher {
        ...
        private ChecksumHasher(Checksum checksum) {
            this.checksum = Objects.requireNonNull(checksum); // checkNotNull(checksum);
        }
