...
//                    bytes[15], bytes[14], bytes[13], bytes[12], bytes[11], bytes[10], bytes[9], bytes[8]);
            return (bytes[15] & 0xFFL) << 56 | (bytes[14] & 0xFFL) << 48 | (bytes[13] & 0xFFL) << 40 |
                    (bytes[12] & 0xFFL) << 32 | (bytes[11] & 0xFFL) << 24 | (bytes[10] & 0xFFL) << 16 |
                    (bytes[9] & 0xFFL) << 8 | (bytes[8] & 0xFFL);
        }
    };
...
//                    new AtomicLongArray(Ints.checkedCast(LongMath.divide(bits, 64, RoundingMode.CEILING)));
            long size = (bits - 1) / 64 + 1;
            this.data = new AtomicLongArray((int) size);
...
//                    other.data.length());
            if (data.length() != other.data.length()) {
                throw new IllegalArgumentException(String.format("BitArrays must be of equal length (%s != %s)", data.length(), other.data.length()));
            }
