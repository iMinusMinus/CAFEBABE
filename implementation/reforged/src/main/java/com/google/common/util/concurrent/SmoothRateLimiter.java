...
//        this.nextFreeTicketMicros = LongMath.saturatedAdd(nextFreeTicketMicros, waitMicros);
        long naiveSum = nextFreeTicketMicros + waitMicros;
        if ((nextFreeTicketMicros ^ waitMicros) < 0 | (nextFreeTicketMicros ^ naiveSum) >= 0) {
            this.nextFreeTicketMicros = naiveSum;
        } else {
            this.nextFreeTicketMicros = Long.MAX_VALUE + ((naiveSum >>> (Long.SIZE - 1)) ^ 1);
        }
