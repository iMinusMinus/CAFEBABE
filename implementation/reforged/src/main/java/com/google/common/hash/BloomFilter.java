...
public final class BloomFilter<T extends /* @Nullable */ Object> implements Predicate<T>, Serializable {
    ...
    private BloomFilter(
        ...
        this.bits = Objects.requireNonNull(bits); // checkNotNull(bits);
        this.numHashFunctions = numHashFunctions;
        this.funnel = Objects.requireNonNull(funnel); // checkNotNull(funnel);
        this.strategy = Objects.requireNonNull(strategy); // checkNotNull(strategy);
    }
...
//        return DoubleMath.roundToLong(-Math.log1p(-fractionOfBitsSet) * bitSize / numHashFunctions, RoundingMode.HALF_UP);
        ...
        if (Math.getExponent(x) > Double.MAX_EXPONENT) {
...
//    @VisibleForTesting
    ...
    public boolean isCompatible(BloomFilter<T> that) {
        Objects.requireNonNull(that); // checkNotNull(that);
        return this != that
    ...
    public void putAll(BloomFilter<T> that) {
        Objects.requireNonNull(that); // checkNotNull(that);
        if (this == that) {
            throw new IllegalArgumentException("Cannot combine a BloomFilter with itself.");
        }
        if (this.numHashFunctions != that.numHashFunctions) {
            throw new IllegalArgumentException(
                    String.format("BloomFilters must have the same number of hash functions (%s != %s)",
                            this.numHashFunctions,
                            that.numHashFunctions));
        }
        if (this.bitSize() != that.bitSize()) {
            throw new IllegalArgumentException(
                    String.format("BloomFilters must have the same size underlying bit arrays (%s != %s)",
                            this.bitSize(),
                            that.bitSize()));
        }
        if (!this.strategy.equals(that.strategy)) {
            throw new IllegalArgumentException(
                    String.format("BloomFilters must have equal strategies (%s != %s)",
                            this.strategy,
                            that.strategy));
        }
        if (!this.funnel.equals(that.funnel)) {
            throw new IllegalArgumentException(
                    String.format("BloomFilters must have equal funnels (%s != %s)",
                            this.funnel,
                            that.funnel));
        }
...
//        return Objects.hashCode(numHashFunctions, funnel, strategy, bits);
    ...
    public static <T extends /* @Nullable */ Object> Collector<T, ?, BloomFilter<T>> toBloomFilter(
        ...
        Objects.requireNonNull(funnel); // checkNotNull(funnel);
        if (expectedInsertions < 0) {
            throw new IllegalArgumentException(String.format("Expected insertions (%s) must be >= 0", expectedInsertions));
        }
        if (fpp <= 0.0) {
            throw new IllegalArgumentException(String.format("False positive probability (%s) must be > 0.0", fpp));
        }
        if (fpp >= 1.0) {
            throw new IllegalArgumentException(String.format("False positive probability (%s) must be < 1.0", fpp));
        }
        return Collector.of(
...
//    @VisibleForTesting
    static <T extends /* @Nullable */ Object> BloomFilter<T> create(
        ...
        Objects.requireNonNull(strategy); // checkNotNull(strategy);
...
//        dout.writeByte(SignedBytes.checkedCast(strategy.ordinal()));
        if (strategy.ordinal() > Byte.MAX_VALUE || strategy.ordinal() < Byte.MIN_VALUE) {
            throw new IllegalArgumentException(String.format("Out of range: %s", strategy.ordinal()));
        }
        dout.writeByte((byte) strategy.ordinal());
//        dout.writeByte(UnsignedBytes.checkedCast(numHashFunctions)); // note: checked at the c'tor
        if (numHashFunctions >> Byte.SIZE != 0) {
            throw new IllegalArgumentException(String.format("out of range: %s", numHashFunctions));
        }
        dout.writeByte((byte) numHashFunctions);
    ...
    public static <T extends /* @Nullable */ Object> BloomFilter<T> readFrom(
        ...
        Objects.requireNonNull(in, "InputStream"); // checkNotNull(in, "InputStream");
        Objects.requireNonNull(funnel, "Funnel"); // checkNotNull(funnel, "Funnel");
        ...
        try {
            ...
            numHashFunctions = din.readByte() & 0xFF; // numHashFunctions = UnsignedBytes.toInt(din.readByte());
            dataLength = din.readInt();

            Strategy strategy = BloomFilterStrategies.values()[strategyOrdinal];

//            LockFreeBitArray dataArray = new LockFreeBitArray(LongMath.checkedMultiply(dataLength, 64L));
            long bits = dataLength * 64L;
            if ((bits > 0 && dataLength < 0) || (bits < 0 && dataLength > 0)) {
                throw new ArithmeticException("overflow: " + dataLength + " * 64");
            }
            LockFreeBitArray dataArray = new LockFreeBitArray(bits);
