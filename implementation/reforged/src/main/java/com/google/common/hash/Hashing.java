...
public final class Hashing {
    ...
    public static HashFunction hmacMd5(byte[] key) {
        return hmacMd5(new SecretKeySpec(Objects.requireNonNull(key), "HmacMD5"));
    }
    ...
    public static HashFunction hmacSha1(byte[] key) {
        return hmacSha1(new SecretKeySpec(Objects.requireNonNull(key), "HmacSHA1"));
    }
    ...
    public static HashFunction hmacSha256(byte[] key) {
        return hmacSha256(new SecretKeySpec(Objects.requireNonNull(key), "HmacSHA256"));
    }
    ...
    public static HashFunction hmacSha512(byte[] key) {
        return hmacSha512(new SecretKeySpec(Objects.requireNonNull(key), "HmacSHA512"));
    }
...
//    @Immutable
    private enum Crc32CSupplier implements /*Immutable*/Supplier<HashFunction> {
//        @J2ObjCIncompatible
        JAVA_UTIL_ZIP {
            @Override
...
//    @Immutable
    enum ChecksumType implements /*Immutable*/Supplier<Checksum> {
...
//        @IgnoreJRERequirement // https://github.com/mojohaus/animal-sniffer/issues/67
        static Checksum newCrc32c() {
            ...
            } catch (RuntimeException | Error re) { // throwIfUnchecked(e);
                throw re;
            } catch (Throwable e) {
    ...
    public static int consistentHash(long input, int buckets) {
        if (buckets <= 0) {
            throw new IllegalArgumentException(String.format("buckets must be positive: %s", buckets));
        }
    ...
    public static HashCode combineOrdered(Iterable<HashCode> hashCodes) {
        ...
        if (!iterator.hasNext()) {
            throw new IllegalArgumentException("Must be at least 1 hash code to combine.");
        }
        ...
        for (HashCode hashCode : hashCodes) {
            ...
            if (nextBytes.length != resultBytes.length) {
                throw new IllegalArgumentException("All hashcodes must have the same bit length.");
            }
    ...
    public static HashFunction concatenating(Iterable<HashFunction> hashFunctions) {
        ...
        if (list.isEmpty()) {
            throw new IllegalArgumentException(String.format("number of hash functions (%s) must be > 0", list.size()));
        }
        return new ConcatenatedHashFunction(list.toArray(new HashFunction[0]));
    }
        ...
        private ConcatenatedHashFunction(HashFunction... functions) {
            ...
            for (HashFunction function : functions) {
                if (function.bits() % 8 != 0) {
                    String errorMessage = String.format("the number of bits (%s) in hashFunction (%s) must be divisible by 8", function.bits(), function);
