...
//        Preconditions.checkPositionIndexes(offset, offset + maxLength, dest.length);
        if (offset < 0 || maxLength < 0 || offset + maxLength > dest.length) {
...
//                    bytes.length);
            if (bytes.length < 4) {
                throw new IllegalStateException(String.format("HashCode#asInt() requires >= 4 bytes (it only has %s bytes).", bytes.length));
            }
            return (bytes[0] & 0xFF)
                    | ((bytes[1] & 0xFF) << 8)
                    | ((bytes[2] & 0xFF) << 16)
...
//                    bytes.length);
            if (bytes.length < 8) {
                throw new IllegalStateException(String.format("HashCode#asLong() requires >= 8 bytes (it only has %s bytes).", bytes.length));
            }
            return padToLong();
        }
