...
//        checkArgument(bufferSize % chunkSize == 0);
    ...
    protected void processRemaining(ByteBuffer bb) {
        bb.position(bb.limit()); // Java8Compatibility.position(bb, bb.limit()); // move at the end
        bb.limit(chunkSize + 7); // Java8Compatibility.limit(bb, chunkSize + 7); // get ready to pad with longs
        while (bb.position() < chunkSize) {
            bb.putLong(0);
        }
        bb.limit(chunkSize); // Java8Compatibility.limit(bb, chunkSize);
        bb.flip(); // Java8Compatibility.flip(bb);
        process(bb);
    }
...
//    @CanIgnoreReturnValue
    ...
    public final HashCode hash() {
        ...
        if (buffer.remaining() > 0) {
            ...
            buffer.position(buffer.limit()); // Java8Compatibility.position(buffer, buffer.limit());
        }
        return makeHash();
    }
