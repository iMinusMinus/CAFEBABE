...
public abstract class Primes
{
    ...
    public static STOutput generateSTRandomPrime(Digest hash, int length, byte[] inputSeed)
    {
        ...
        return implSTRandomPrime(hash, length, inputSeed.clone());
    }
    ...
    public static MROutput enhancedMRProbablePrimeTest(BigInteger candidate, SecureRandom random, int iterations)
    {
        ...
        for (int i = 0; i < iterations; ++i)
        {
            ...
            if (!primeToBase)
            {
                ...
                g = x.subtract(BigInteger.ONE).gcd(w);
    ...
    public static boolean isMRProbablePrimeToBase(BigInteger candidate, BigInteger base)
    {
        ...
        if (base.compareTo(candidate.subtract(BigInteger.ONE)) >= 0)
        {
    ...
    private static STOutput implSTRandomPrime(Digest d, int length, byte[] primeSeed)
    {
        ...
        x = x.mod(BigInteger.ONE.shiftLeft(length - 1)).setBit(length - 1);

        BigInteger c0x2 = c0.shiftLeft(1);
        BigInteger tx2 = x.subtract(BigInteger.ONE).divide(c0x2).add(BigInteger.ONE).shiftLeft(1);
        int dt = 0;

        BigInteger c = tx2.multiply(c0).add(BigInteger.ONE);
        ...
        for (;;)
        {
            ...
            {
                tx2 = BigInteger.ONE.shiftLeft(length - 1).subtract(BigInteger.ONE).divide(c0x2).add(BigInteger.ONE).shiftLeft(1);
                c = tx2.multiply(c0).add(BigInteger.ONE);
            }
            ...
            if (!implHasAnySmallFactors(c))
            {
                ...
                if (c.gcd(z.subtract(BigInteger.ONE)).equals(BigInteger.ONE) && z.modPow(c0, c).equals(BigInteger.ONE))
                {
