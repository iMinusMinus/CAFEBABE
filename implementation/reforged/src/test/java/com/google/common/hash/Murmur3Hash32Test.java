...
public class Murmur3Hash32Test /* extends TestCase */ {
    ...
    @Test public void testKnownStringInputs() {
    ...
    @Test public void testKnownEncodedStringInputs() {
    ...
    @SuppressWarnings("deprecation")
    @Test public void testSimpleStringUtf8() {
    ...
    @SuppressWarnings("deprecation")
    @Test public void testEncodedStringInputs() {
        ...
        for (int z = 0; z < 100; z++) {
            ...
            Assertions.assertEquals(hashUtf8, murmur3_32().newHasher().putBytes(str.getBytes(UTF_8)).hash());
            Assertions.assertEquals(hashUtf8, murmur3_32().hashString(str, UTF_8));
            Assertions.assertEquals(hashUtf8, murmur3_32().newHasher().putString(str, UTF_8).hash());
            HashCode hashUtf16 = murmur3_32().hashBytes(str.getBytes(UTF_16));
            Assertions.assertEquals(hashUtf16, murmur3_32().newHasher().putBytes(str.getBytes(UTF_16)).hash());
            Assertions.assertEquals(hashUtf16, murmur3_32().hashString(str, UTF_16));
            Assertions.assertEquals(hashUtf16, murmur3_32().newHasher().putString(str, UTF_16).hash());
        }
    }

    private static void assertHash(int expected, HashCode actual) {
        Assertions.assertEquals(HashCode.fromInt(expected), actual);
    }

    @Test public void testParanoidHashBytes() {
        HashFn hf =
                new HashFn() {
                    @Override
    ...
    @Test public void testInvariants() {
        HashTestUtils.assertInvariants(murmur3_32());
    }

    @SuppressWarnings("deprecation")
    @Test public void testInvalidUnicodeHashString() {
        String str =
                new String(
                        new char[] {'a', Character.MIN_HIGH_SURROGATE, Character.MIN_HIGH_SURROGATE, 'z'});
        Assertions.assertEquals(murmur3_32().hashBytes(str.getBytes(UTF_8)), murmur3_32().hashString(str, UTF_8));
        Assertions.assertEquals(
                murmur3_32_fixed().hashBytes(str.getBytes(UTF_8)), murmur3_32().hashString(str, UTF_8));
    }

    @SuppressWarnings("deprecation")
    @Test public void testInvalidUnicodeHasherPutString() {
        String str =
                new String(
                        new char[] {'a', Character.MIN_HIGH_SURROGATE, Character.MIN_HIGH_SURROGATE, 'z'});
        Assertions.assertEquals(
