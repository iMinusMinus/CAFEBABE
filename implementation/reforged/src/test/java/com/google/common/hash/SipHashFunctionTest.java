...
public class SipHashFunctionTest /* extends TestCase */ {
    ...
    @Test public void test15ByteStringFromSipHashPaper() {
        byte[] message =
                new byte[] {
        ...
        Assertions.assertEquals(0xa129ca6149be45e5L, Hashing.sipHash24(k0, k1).hashBytes(message).asLong());
    }

    // From https://github.com/BrandonHaynes/siphash-csharp/blob/master/tests/Tests.cs
    @Test public void testKnownValues() {
    ...
    @Test public void testCollisionsDueToIncorrectSignExtension() {
        ...
        /* Immutable */
        Set<HashCode> hashCodes = Arrays.asList(SIP_WITH_KEY.hashBytes(col1),
                SIP_WITH_KEY.hashBytes(col2),
                SIP_WITH_KEY.hashBytes(col3)).stream().collect(Collectors.toSet());
//                ImmutableSet.of(
//                        SIP_WITH_KEY.hashBytes(col1),
//                        SIP_WITH_KEY.hashBytes(col2),
//                        SIP_WITH_KEY.hashBytes(col3));
        Assertions.assertEquals(3, hashCodes.size());
    }

    @Test public void testToString() {
        Assertions.assertEquals("Hashing.sipHash24(" + K0 + ", " + K1 + ")", SIP_WITH_KEY.toString());
        Assertions.assertEquals("Hashing.sipHash24(" + K0 + ", " + K1 + ")", SIP_WITHOUT_KEY.toString());
        Assertions.assertEquals("Hashing.sipHash24(20, 13)", Hashing.sipHash24(20, 13).toString());
    }

    private static void assertSip(String input, long expected) {
        Assertions.assertEquals(expected, SIP_WITH_KEY.hashString(input, UTF_8).asLong());
        Assertions.assertEquals(expected, SIP_WITH_KEY.newHasher().putString(input, UTF_8).hash().asLong());
        Assertions.assertEquals(expected, SIP_WITHOUT_KEY.hashString(input, UTF_8).asLong());
        Assertions.assertEquals(expected, SIP_WITHOUT_KEY.newHasher().putString(input, UTF_8).hash().asLong());
    }

    private static void assertSip(byte[] input, long expected) {
        Assertions.assertEquals(expected, SIP_WITH_KEY.hashBytes(input).asLong());
        Assertions.assertEquals(expected, SIP_WITH_KEY.newHasher().putBytes(input).hash().asLong());
        Assertions.assertEquals(expected, SIP_WITHOUT_KEY.hashBytes(input).asLong());
        Assertions.assertEquals(expected, SIP_WITHOUT_KEY.newHasher().putBytes(input).hash().asLong());
    }
}