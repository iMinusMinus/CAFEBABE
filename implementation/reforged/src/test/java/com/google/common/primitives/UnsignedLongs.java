...
//    @CanIgnoreReturnValue
    public static long parseUnsignedLong(String string, int radix) {
        Objects.requireNonNull(string); // checkNotNull(string);
        if (string.length() == 0) {
...
//                radix);
        if (radix < Character.MIN_RADIX || radix > Character.MAX_RADIX) {
            throw new IllegalArgumentException(String.format("radix (%s) must be between Character.MIN_RADIX and Character.MAX_RADIX", radix));
        }
        if (x == 0) {
            // Simply return "0"
            return "0";
