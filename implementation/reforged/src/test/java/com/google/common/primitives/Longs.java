...
//    @InlineMe(replacement = "Long.compare(a, b)")
     ...
     */
    public static int indexOf(long[] array, long[] target) {
        Objects.requireNonNull(array, "array"); // checkNotNull(array, "array");
        Objects.requireNonNull(target, "target"); // checkNotNull(target, "target");
        if (target.length == 0) {
            return 0;
        }
...
//                result);
        if (result != (int) result) {
            throw new IllegalArgumentException(String.format("the total number of elements (%s) in the arrays must fit in an int", result));
        }
        return (int) result;
    }
...
//        checkArgument(bytes.length >= BYTES, "array too small: %s < %s", bytes.length, BYTES);
        if (bytes.length < BYTES) {
            throw new IllegalArgumentException(String.format("array too small: %s < %s", bytes.length, BYTES));
        }
        return fromBytes(
...
//    @CheckForNull
    public static Long tryParse(String string, int radix) {
        if (Objects.requireNonNull(string).isEmpty()) { // checkNotNull(string).isEmpty()
            return null;
        }
...
//        checkArgument(padding >= 0, "Invalid padding: %s", padding);
    ...
    public static String join(String separator, long... array) {
        Objects.requireNonNull(separator); // checkNotNull(separator);
        if (array.length == 0) {
            return "";
        }
    ...
    public static void sortDescending(long[] array) {
        Objects.requireNonNull(array); // checkNotNull(array);
...
//            array[i] = ((Number) checkNotNull(boxedArray[i])).longValue();
            array[i] = ((Number) Objects.requireNonNull(boxedArray[i])).longValue();
        }
        return array;
    }
...
//            checkElementIndex(index, size());
            ...
            // checkNotNull for GWT (do not optimize)
            array[start + index] = Objects.requireNonNull(element); // checkNotNull(element);
            return oldValue;
        }
