...
public final class EqualsTester {
    ...
    private final List<List<Object>> equalityGroups = new ArrayList<>(); // Lists.newArrayList();
...
//    @CanIgnoreReturnValue
    public EqualsTester addEqualityGroup(/* @Nullable */ Object /* @Nullable */ ... equalityGroup) {
        Objects.requireNonNull(equalityGroup); // checkNotNull(equalityGroup);
...
//        for (Object item : Iterables.concat(equalityGroups)) {
        ...
        equalityGroups.forEach(list -> all.addAll(list));
            for (Object item : all) {
            Assertions.assertTrue(!item.equals(null), item + " must not be Object#equals to null");
            Assertions.assertTrue(!item.equals(NotAnInstance.EQUAL_TO_NOTHING),
                    item + " must not be Object#equals to an arbitrary object of another class");
            Assertions.assertTrue(item.equals(item), item + " must be Object#equals to itself");
            Assertions.assertEquals(item.hashCode(), item.hashCode(),
                    "the Object#hashCode of " + item + " must be consistent");
            if (!(item instanceof String)) {
                Assertions.assertTrue(!item.equals(item.toString()),
