...
public class FarmHashFingerprint64Test /* extends TestCase */ {
    ...
    @Test public void testReallySimpleFingerprints() {
        Assertions.assertEquals(8581389452482819506L, fingerprint("test".getBytes(UTF_8)));
        // 32 characters long
        Assertions.assertEquals(-4196240717365766262L, fingerprint(repeat("test", 8).getBytes(UTF_8)));
        // 256 characters long
        Assertions.assertEquals(3500507768004279527L, fingerprint(repeat("test", 64).getBytes(UTF_8)));
    }
...
//                .isNotEqualTo(fingerprint(stringB.getBytes(UTF_8)));
    ...
    @Test public void testHashFloatIsStable() {
        ...
        Assertions.assertEquals(0x49f9d18ee8ae1b28L, hasher.hash().asLong());

        hasher = HASH_FN.newHasher();
        hasher.putDouble(0x0000000001000101d);
        Assertions.assertEquals(0x388ee898bad75cbfL, hasher.hash().asLong());
    }
    ...
    @Test public void testMultipleLengths() {
        ...
        Assertions.assertEquals(0x7a1d67c50ec7e167L, h);
    }
