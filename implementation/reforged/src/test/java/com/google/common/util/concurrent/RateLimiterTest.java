...
public class RateLimiterTest /* extends TestCase */ {
    ...
    @Test public void testImmediateTryAcquire() {
        RateLimiter r = RateLimiter.create(1);
        Assertions.assertTrue(r.tryAcquire(), "Unable to acquire initial permit");
        Assertions.assertFalse(r.tryAcquire(), "Capable of acquiring secondary permit");
    }

    @Test public void testDoubleMinValueCanAcquireExactlyOnce() {
        RateLimiter r = RateLimiter.create(Double.MIN_VALUE, stopwatch);
        Assertions.assertTrue(r.tryAcquire(), "Unable to acquire initial permit");
        Assertions.assertFalse(r.tryAcquire(), "Capable of acquiring an additional permit");
        stopwatch.sleepMillis(Integer.MAX_VALUE);
        Assertions.assertFalse(r.tryAcquire(), "Capable of acquiring an additional permit after sleeping");
    }

    @Test public void testSimpleRateUpdate() {
        RateLimiter limiter = RateLimiter.create(5.0, 5, SECONDS);
//        assertThat(limiter.getRate()).isEqualTo(5.0);
        Assertions.assertEquals(5.0, limiter.getRate());
        limiter.setRate(10.0);
//        assertThat(limiter.getRate()).isEqualTo(10.0);
        Assertions.assertEquals(10.0, limiter.getRate());

        Assertions.assertThrows(IllegalArgumentException.class, () -> limiter.setRate(0.0));
        Assertions.assertThrows(IllegalArgumentException.class, () -> limiter.setRate(-10.0));
        Assertions.assertThrows(IllegalArgumentException.class, () -> limiter.setRate(Double.NaN));
    }

    @Test public void testAcquireParameterValidation() {
        RateLimiter limiter = RateLimiter.create(999);
        Assertions.assertThrows(IllegalArgumentException.class, () -> limiter.acquire(0));
        Assertions.assertThrows(IllegalArgumentException.class, () -> limiter.acquire(-1));
        Assertions.assertThrows(IllegalArgumentException.class, () -> limiter.tryAcquire(0));
        Assertions.assertThrows(IllegalArgumentException.class, () -> limiter.tryAcquire(-1));
        Assertions.assertThrows(IllegalArgumentException.class, () -> limiter.tryAcquire(0, 1, SECONDS));
        Assertions.assertThrows(IllegalArgumentException.class, () -> limiter.tryAcquire(-1, 1, SECONDS));
    }

    @Test public void testSimpleWithWait() {
        RateLimiter limiter = RateLimiter.create(5.0, stopwatch);
    ...
    @Test public void testSimpleAcquireReturnValues() {
        RateLimiter limiter = RateLimiter.create(5.0, stopwatch);
//        assertThat(limiter.acquire()).isWithin(EPSILON).of(0.0); // R0.00
        Assertions.assertTrue(Math.abs(limiter.acquire() - 0.0) < EPSILON);
        stopwatch.sleepMillis(200); // U0.20, we are ready for the next request...
...
//        assertThat(limiter.acquire()).isWithin(EPSILON).of(0.2); // R0.20
        Assertions.assertTrue(Math.abs(limiter.acquire() - 0.2) < EPSILON);
        assertEvents("R0.00", "U0.20", "R0.00", "R0.20");
    }

    @Test public void testSimpleAcquireEarliestAvailableIsInPast() {
        RateLimiter limiter = RateLimiter.create(5.0, stopwatch);
...
//        assertThat(limiter.acquire()).isWithin(EPSILON).of(0.2);
    ...
    @Test public void testOneSecondBurst() {
        RateLimiter limiter = RateLimiter.create(5.0, stopwatch);
    ...
    @Test public void testCreateWarmupParameterValidation() {
        RateLimiter unused;
        unused = RateLimiter.create(1.0, 1, NANOSECONDS);
        unused = RateLimiter.create(1.0, 0, NANOSECONDS);

        Assertions.assertThrows(IllegalArgumentException.class, () -> RateLimiter.create(0.0, 1, NANOSECONDS));

        Assertions.assertThrows(IllegalArgumentException.class, () -> RateLimiter.create(1.0, -1, NANOSECONDS));
    }

//    @AndroidIncompatible // difference in String.format rounding?
    @Test public void testWarmUp() {
    ...
    @Test public void testWarmUpWithColdFactor() {
    ...
    @Test public void testWarmUpWithColdFactor1() {
...
//    @AndroidIncompatible // difference in String.format rounding?
    @Test public void testWarmUpAndUpdate() {
        RateLimiter limiter = RateLimiter.create(2.0, 4000, MILLISECONDS, 3.0, stopwatch);
        for (int i = 0; i < 8; i++) {
            limiter.acquire(); // // #1
        }
    ...
    @Test public void testWarmUpAndUpdateWithColdFactor() {
        RateLimiter limiter = RateLimiter.create(5.0, 4000, MILLISECONDS, 10.0, stopwatch);
        for (int i = 0; i < 8; i++) {
            limiter.acquire(); // #1
        }
    ...
    @Test public void testBurstyAndUpdate() {
    ...
    @Test public void testTryAcquire_noWaitAllowed() {
        RateLimiter limiter = RateLimiter.create(5.0, stopwatch);
        Assertions.assertTrue(limiter.tryAcquire(0, SECONDS));
        Assertions.assertFalse(limiter.tryAcquire(0, SECONDS));
        Assertions.assertFalse(limiter.tryAcquire(0, SECONDS));
        stopwatch.sleepMillis(100);
        Assertions.assertFalse(limiter.tryAcquire(0, SECONDS));
    }

    @Test public void testTryAcquire_someWaitAllowed() {
        RateLimiter limiter = RateLimiter.create(5.0, stopwatch);
        Assertions.assertTrue(limiter.tryAcquire(0, SECONDS));
        Assertions.assertTrue(limiter.tryAcquire(200, MILLISECONDS));
        Assertions.assertFalse(limiter.tryAcquire(100, MILLISECONDS));
        stopwatch.sleepMillis(100);
        Assertions.assertTrue(limiter.tryAcquire(100, MILLISECONDS));
    }

    @Test public void testTryAcquire_overflow() {
        RateLimiter limiter = RateLimiter.create(5.0, stopwatch);
        Assertions.assertTrue(limiter.tryAcquire(0, MICROSECONDS));
        stopwatch.sleepMillis(100);
        Assertions.assertTrue(limiter.tryAcquire(Long.MAX_VALUE, MICROSECONDS));
    }

    @Test public void testTryAcquire_negative() {
        RateLimiter limiter = RateLimiter.create(5.0, stopwatch);
        Assertions.assertTrue(limiter.tryAcquire(5, 0, SECONDS));
        stopwatch.sleepMillis(900);
        Assertions.assertFalse(limiter.tryAcquire(1, Long.MIN_VALUE, SECONDS));
        stopwatch.sleepMillis(100);
        Assertions.assertTrue(limiter.tryAcquire(1, -1, SECONDS));
    }

    @Test public void testSimpleWeights() {
        RateLimiter rateLimiter = RateLimiter.create(1.0, stopwatch);
        rateLimiter.acquire(1); // no wait
        rateLimiter.acquire(1); // R1.00, to repay previous
    ...
    @Test public void testInfinity_bursty() {
    ...
    @Test public void testInfinity_bustyTimeElapsed() {
        RateLimiter limiter = RateLimiter.create(Double.POSITIVE_INFINITY, stopwatch);
    ...
    @Test public void testInfinity_warmUp() {
    ...
    @Test public void testInfinity_warmUpTimeElapsed() {
        RateLimiter limiter = RateLimiter.create(Double.POSITIVE_INFINITY, 10, SECONDS, 3.0, stopwatch);
        stopwatch.instant += 1000000;
        limiter.setRate(1.0);
        for (int i = 0; i < 5; i++) {
     ...
     * we change the rate.
     */
    @Test public void testWeNeverGetABurstMoreThanOneSec() {
        ...
        for (int rate : rates) {
            ...
            Assertions.assertTrue(burst <= 1000);
            ...
            Assertions.assertTrue(afterBurst >= 1000);
        }
    }
     ...
     */
    @Test public void testTimeToWarmUpIsHonouredEvenWithWeights() {
        ...
        for (int trial = 0; trial < 100; trial++) {
            for (double coldFactor : coldFactorsToTest) {
                for (double qps : qpsToTest) {
                    ...
                    Assertions.assertEquals(warmupMillis, measureTotalTimeMillis(rateLimiter, warmupPermits, random));
                }
            }
        }
    }
...
//    }
    ...
    @Test public void testVerySmallDoubleValues() throws Exception {
        RateLimiter rateLimiter = RateLimiter.create(Double.MIN_VALUE, stopwatch);
        Assertions.assertTrue(rateLimiter.tryAcquire(), "Should acquire initial permit");
        Assertions.assertFalse(rateLimiter.tryAcquire(), "Should not acquire additional permit");
        stopwatch.sleepMillis(5000);
        Assertions.assertFalse(rateLimiter.tryAcquire(),
                "Should not acquire additional permit even after sleeping");
    }
    ...
    static class FakeStopwatch extends SleepingStopwatch {
        long instant = 0L;
        final List<String> events = new ArrayList<>(); // Lists.newArrayList();
...
//    @AndroidIncompatible // Mockito loses its ability to mock doGetRate as of Android 21
    @Test public void testMockingMockito() throws Exception {
    ...
    private static final /* Immutable */ Set<String> NOT_WORKING_ON_MOCKS =
//            ImmutableSet.of("latestPermitAgeSec", "latestPermitAge", "setRate", "getAvailablePermits");
    ...
    private static final /* ImmutableClassToInstance */ Map<Class, Object> PARAMETER_VALUES = new HashMap<>();
...
//                    .build();
    static {
        PARAMETER_VALUES.put(int.class, 1);
        PARAMETER_VALUES.put(long.class, 1L);
        PARAMETER_VALUES.put(double.class, 1.0);
        PARAMETER_VALUES.put(TimeUnit.class, SECONDS);
    }
}
